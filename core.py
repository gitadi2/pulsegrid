...
def train_models(df: pd.DataFrame, with_cv: bool = True) -> dict:
    ...
    # 1) clearance-time (duration) regressor -- intake features only
    d = df[df["duration_min"].between(1, 60 * 24 * 7)].copy()
    Xd, yd = _frame(d), np.log1p(d["duration_min"].values)
    dur = _pipe(HistGradientBoostingRegressor(
        max_iter=500, learning_rate=0.05, max_leaf_nodes=31,
        l2_regularization=1.0, random_state=0))
    if with_cv:
        p = cross_val_predict(dur, Xd, yd, cv=KFold(5, shuffle=True, random_state=0))
        t, pe = np.expm1(yd), np.clip(np.expm1(p), 0, None)
        out["metrics"]["duration"] = {
            "n": len(d), "MAE_min": float(mean_absolute_error(t, pe)),
            "median_AE_min": float(np.median(np.abs(t - pe))),
            "R2_log": float(r2_score(yd, p))}
    dur.fit(Xd, yd)
    out["duration"] = dur
    ...
    # 3) road-closure classifier (barricade needed?) -- intake features only
    Xc, yc = _frame(df), df["road_closure"].values
    clo = _pipe(HistGradientBoostingClassifier(
        max_iter=400, learning_rate=0.06, max_leaf_nodes=31,
        class_weight="balanced", random_state=0))
    if with_cv:
        cr = cross_val_predict(clo, Xc, yc, cv=StratifiedKFold(5, shuffle=True, random_state=0),
                               method="predict_proba")[:, 1]
        thr = 0.30
        fl = cr >= thr
        out["metrics"]["closure"] = {
            ...
            "recall_at_30": float(yc[fl].sum() / max(1, yc.sum())),
            "flag_rate_at_30": float(fl.mean())}
    clo.fit(Xc, yc)
    out["closure"] = clo
...
def predict_event(models: dict, ev: dict) -> dict:
    ...
    dur = float(np.clip(np.expm1(models["duration"].predict(X)[0]), 1, None))
    pri_p = float(models["priority"].predict_proba(X)[0, 1])
    clo_p = float(models["closure"].predict_proba(X)[0, 1])
    return {
        "duration_min": dur,
        "clearance_band": ("Quick (<30 min)" if dur < 30 else
                           "Moderate (30 min–2 hr)" if dur < 120 else
                           "Extended (>2 hr)"),
...
def recommend_deployment(pred: dict, corridor: str = "") -> dict:
    ...
    units = min(units, 4)
    diversion = (f"Set up diversion around {corridor}"
