...
2. **Live Triage** — log an incoming incident; get an instant color-coded dispatch ticket
...
3. **Plan Event** — for *planned* congestion (festivals, rallies, VIP movement, processions, roadwork):
   ...
   risk profile.
4. **Hotspot Map** — geospatial view of the real chokepoints to position reserves.
5. **Forecast & Staffing** — 7-day volume forecast with **automatic surge alerts** (days running above
   the mean+1σ norm) and a corridor-level officer allocation for any chosen headcount.
6. **Model Card** — transparent metrics and a **proactive-advantage** statement: the barricade model
